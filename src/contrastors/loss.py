...
def cache_loss(tower1, tower2, query_embeddings, document_embeddings, logit_scale, bidirectional=False, tracker=None, step=None):
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16):
        with no_tower1_sync():
            with no_tower2_sync():
                loss = clip_loss(query_embs, document_embs, logit_scale, gather_enabled=True, bidirectional=bidirectional, tracker=tracker, step=step)
                loss.backward()
...
def grad_cache_loss(tower1, t1_inputs, tower2, t2_inputs, chunk_size, logit_scale, bidirectional=False, router_aux_coeff=False, tracker=None, step=None, **kwargs):
    ...
    query_cache, document_cache, loss = cache_loss(
        tower1, tower2, query_embs, document_embs, logit_scale, bidirectional=bidirectional, tracker=tracker, step=step
    )
