...
__all__ = [
    ...
    "modernbert_config_to_nomic_config",
...
def modernbert_config_to_nomic_config(bert_config: ModernBertConfig) -> NomicBertConfig:
    return NomicBertConfig(
        vocab_size=bert_config.vocab_size,
        n_positions=bert_config.max_position_embeddings,  # No absolute position embedding
        n_embd=bert_config.hidden_size,
        ...
        n_head=bert_config.num_attention_heads,
        n_inner=bert_config.intermediate_size,
        activation_function=bert_config.hidden_activation,
        embd_pdrop=bert_config.embedding_dropout,
        attn_pdrop=bert_config.attention_dropout,
        layer_norm_epsilon=bert_config.layer_norm_eps,
        initializer_range=bert_config.initializer_range,
        bos_token_id=bert_config.bos_token_id,
        eos_token_id=bert_config.eos_token_id,
        resid_pdrop=0.0, # No dropout
        rotary_emb_base=bert_config.local_rope_theta,
        # These are new arguments not in the original GPT2Config
        prenorm=False,
        parallel_block=False,
        parallel_block_tied_norm=False,
        ...
        mlp_fc1_bias=getattr(bert_config, "mlp_fc1_bias", True),
        mlp_fc2_bias=getattr(bert_config, "mlp_fc2_bias", True),
        use_rms_norm=getattr(bert_config, "use_rms_norm", True),
        causal=False,
        type_vocab_size=2,
        dense_seq_output=True,
        pad_vocab_size_multiple=getattr(bert_config, "pad_vocab_to_multiple_of", 1),
...
def remap_bert_state_dict(
    ...
    # def remap_modernbert_prefix(key):
    #     return re.sub(r"^model.", "bert.encoder.", key)
    # state_dict = OrderedDict((remap_modernbert_prefix(k), v) for k, v in state_dict.items())
    ...
    if pad_vocab_size_multiple > 1:
        ...
        if not remove_cls_weights and "cls.predictions.decoder.weight" in state_dict:
