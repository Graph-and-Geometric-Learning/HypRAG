...
from models.encoder.bert import remap_bert_state_dict
from models.encoder.configuration_nomic_bert import NomicBertConfig
...
class NomicBertForPreTraining(NomicBertPreTrainedModel):
    ...
    def forward(
        ...
        # if input_len > self.config.n_positions:
        #     input_ids = input_ids[:, :self.config.n_positions]
        #     if position_ids is not None:
        #         position_ids = position_ids[:, :self.config.n_positions]
        #     if attention_mask is not None:
        #         attention_mask = attention_mask[:, :self.config.n_positions]
        #     if token_type_ids is not None:
        #         token_type_ids = token_type_ids[:, :self.config.n_positions]
        ...
        seq_norm = torch.norm(sequence_output, dim=-1, keepdim=True)
