...
from loss import clip_loss, grad_cache_loss, calculate_auxiliary_loss
from models import BiEncoder, BiEncoderConfig, LogitScale, load_tokenizer
...
class SentenceTransformerModule(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor], **kwargs) -> Dict[str, torch.Tensor]:
        ...
        return {"sentence_embedding": embeddings["last_hidden_state"]}
...
class TextTextTrainer(BaseTrainer):
    ...
    def _forward_step(self, model, batch, logit_scale, matryoshka_dims=None, matroyshka_loss_weights=None, **kwargs):
        ...
        if hasattr(self.config.model_args, "num_experts") and self.config.model_args.num_experts > 0:
            if hasattr(self.config.train_args, "router_aux_loss_coef") and self.config.train_args.router_aux_loss_coef > 0:
        ...
        if hasattr(self.config.model_args, "num_experts") and self.config.model_args.num_experts > 0 \
                and hasattr(self.config.train_args, "router_aux_loss_coef") and self.config.train_args.router_aux_loss_coef > 0:
