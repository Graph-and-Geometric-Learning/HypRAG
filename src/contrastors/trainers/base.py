...
add_safe_globals([fragment_address])
...
class BaseTrainer(metaclass=ABCMeta):
    ...
    def get_trackers(self, config):
        ...
        if slurm_job_id:
            run_name = f"{run_name}_{slurm_job_id}"
        ...
        if self.global_rank == 0:
            ...
            wandb_mode = config.train_args.wandb_mode
            ...
            # tracker = wandb.init(project=project_name, name=run_name, config=hyperparams, group=group)
    ...
    def get_model(self, config):
        ...
        model_dict = load_from_config(config)
    ...
    def get_scheduler(self, config, optimizer, ds_config):
        if hasattr(config, "warmup_steps") and getattr(config, "warmup_steps") is not None:
            total_num_steps = self.total_num_steps * config.num_epochs // config.gradient_accumulation_steps
    ...
    def save_model(self, output_dir):
        if self.global_rank == 0:
            ...
            unwrapped.save_pretrained(output_dir, state_dict=state_dict, safe_serialization=False)
    ...
    def load_state(self, input_dir):
        ...
        random_states = torch.load(f"{input_dir}/random_states_{self.process_index}.pt", weights_only=False)
    ...
    def save_state(self, output_dir):
        ...
        self.print(f"Saving model to {output_dir}/model")
        ...
        if self.deepspeed:
            ...
            if isinstance(self.dataloaders["train"].dataset, StreamingShardDataset):
                train_args = self.config.train_args
                data_config = self.config.data_args
                path = data_config.input_shards.replace(".yaml", "")
                path = os.path.join(train_args.output_dir, path) if train_args.output_dir else path
                with open(f"{path}/rank_{dist.get_rank()}_processed.json", "r") as f:
                    processed = json.load(f)

                print(f"Saving processed data to {output_dir}/rank_{dist.get_rank()}_processed.json")
        ...
        if save_total_limit is not None and save_total_limit > 0 and self.global_rank == 0:
            existing_dirs = [d for d in os.listdir(parent_dir) if os.path.isdir(os.path.join(parent_dir, d)) and d.startswith("step_") or d.startswith("epoch_")]
            if len(existing_dirs) >= save_total_limit:
                # sort directories by modification time
                existing_dirs.sort(key=lambda x: os.path.getmtime(os.path.join(parent_dir, x)))
                dirs_to_remove = existing_dirs[:len(existing_dirs) - save_total_limit + 1]
                for dir_to_remove in dirs_to_remove:
                    dir_path = os.path.join(parent_dir, dir_to_remove)
                    self.print(f"Removing old checkpoint directory: {dir_path}")
    ...
    def training_step(
        ...
        with torch.autocast(device_type="cuda", dtype=self.dtype, enabled=(not self.deepspeed and not self.config.train_args.grad_cache)):
            fw_output = self.forward_step(inputs=batch, **model, step=step)

        self.backward(fw_output['loss'])

        # self.engine.backward(fw_output['loss'])
        # # print gradient norm before clipping
        # gradient_norms = {}
        # with deepspeed.zero.GatheredParameters(model['model'].parameters(), modifier_rank=0):
        #     for name, p in model['model'].named_parameters():
        #         if p.grad is not None:
        #             g = p.grad
        #             if name not in gradient_norms:
        #                 gradient_norms[f"grad_{name}"] = g.norm().item()
        # fw_output.update(gradient_norms)
        # self.engine.step()
    ...
    def train(self):
                ...
                for step, batch in enumerate(progbar):
                    ...
                    if isinstance(loss, torch.Tensor):
                        ...
                        for k, v in loss.items():
                            if isinstance(v, torch.Tensor):
                                ...
                                metrics[k] = torch.tensor(v).float().to("cuda")
                            else:
                                raise TypeError(f"Unexpected loss type: {type(v)}")
                    else:
                    ...
                    if train_args.wandb:
                        if step % gradient_accumulation_steps == 0 and step > 0 or step == total_training_steps - 1:
                            self.log({k: torch.mean(v).item() for k, v in metrics.items()}, step=(curr_step // gradient_accumulation_steps))
                    else:
                    ...
                    if val_dataloader is not None:
                        if (
                            ...
                            self.eval_loop(dataloader=val_dataloader, step=(curr_step // gradient_accumulation_steps), **model)

                    # log LR in case something weird happens
                    if step > 0 and step % (train_args.log_lr_every) == 0:
                        if train_args.wandb:
                            if step % gradient_accumulation_steps == 0 and step > 0 or step == total_training_steps - 1:
                                self.log({"lr": scheduler.get_last_lr()[0]}, step=(curr_step // gradient_accumulation_steps))
        ...
        if val_dataloader is not None and train_args.eval_strategy not in ["no", None]:
            self.eval_loop(dataloader=val_dataloader, step=(curr_step // gradient_accumulation_steps), **model)
