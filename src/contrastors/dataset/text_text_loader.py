...
from datasets import load_dataset, interleave_datasets, load_dataset_builder, concatenate_datasets
...
class StreamingShardDataset(IterableDataset):
    def __init__(
        ...
        if self.run_name is None:
            self.run_name = os.environ.get("SLURM_JOB_ID", "local")
        else:
            self.run_name = f"{self.run_name}_{slurm_job_id}"

        self.temp_dir = Path(f"/tmp/{self.run_name}")
        ...
        self.filesystem = "s3"
        self.fs = fsspec.filesystem(self.filesystem, anon=True, config_kwargs={"connect_timeout": 600, "read_timeout": 600})
        ...
        path = os.path.join(self.output_dir, path) if self.output_dir else path
    ...
    def parse_spec(self, fname):
        ...
        for ds in spec["datasets"]:
            ...
            tmp_dir = Path(self.temp_dir) / Path(f"{bucket.replace('s3://', '')}")
    ...
    def load_state(self, path):
        ...
        """
        if not os.path.exists(os.path.join(path, f"rank_{self.rank}_processed.json")):
            print(f"Path {path} does not exist, cannot load state, skipping")
            return
    ...
    def download_rank_zero(self, s3_path):
        ...
        local_path = Path(self.temp_dir) / Path(f"{s3_path.replace('s3://', '')}")
...
class LocalShardDataset(Dataset):
    def __init__(self, ds_spec, num_negatives=0, seed=42, data_path='/tmp'):
        if dist.is_initialized():
            self.rank = dist.get_rank()
            self.world_size = dist.get_world_size()
        else:
    ...
    def download_rank_zero(self, s3_path):
        # only download if we are rank 0 otherwise wait for rank 0 to download
        # TODO: fix for multinode, need to update the if statement
        local_path = Path(self.data_path) / Path(f"{s3_path.replace('s3://', '')}")
        if self.rank == 0:
            if not local_path.parent.exists():
                local_path.parent.mkdir(parents=True, exist_ok=True)
            if not local_path.exists():
    ...
    def load_examples(self, paths):
        ...
        for path in tqdm(paths, desc="Loading examples", disable=self.rank > 0):
            ...
            contrastive_type_metadata = self.path2objective[str(path).replace(f"{self.data_path}/", "")]
...
class LocalHFDataset(Dataset):
    def __init__(self, config_file, tokenizer, num_negatives=0, add_prefix=True, add_eos=False, data_path=None, num_workers=0, query_max_length=None, document_max_length=None):
        ...
        self.add_eos = add_eos
        self.max_lengths = {
            "query": query_max_length or DEFAULT_COL_TO_MAX_TOKENS["query"],
            "document": document_max_length or DEFAULT_COL_TO_MAX_TOKENS["document"],
            "negative": document_max_length or DEFAULT_COL_TO_MAX_TOKENS["negative"],
        }

        with open(config_file) as f:
            self.ds_spec = yaml.safe_load(f)

        self.dataset, self.split2prefix = self.load_hf_dataset(self.ds_spec, data_path, num_workers)

    def __len__(self):
        return len(self.dataset)

    def build_paired_data(self, sample):
        """Extracts the relevant pairs from a sample using the metadata provided."""
        negatives = []
        if "negative" in sample and self.num_negatives > 0:
            if len(sample["negative"]) <= self.num_negatives:
                negatives = sample.pop("negative", []) 
            else:
                negatives = random.sample(sample.pop("negative", []), self.num_negatives)

        if "document" in sample:
            sample["document"] = [sample["document"]] + negatives

        return sample

    def preprocess_text(self, text, col, ds_name):
        """Preprocess the text based on the column and dataset name."""
        if self.add_eos:
            text = text + self.tokenizer.eos_token

        if self.add_prefix:
            prefix = self.split2prefix.get(ds_name if ds_name else 'default').get(col, KEY2PREFIX[col])
            text = f"{prefix}: {text}"
    ...
    def __getitem__(self, index):
        ...
        pair = self.build_paired_data(sample)
        ds_name = pair.pop("dataset", 'default')
        tokenized = {}

        contrastive_type = self.ds_spec["objective"]["type"]
        valid_cols = MAPPED_NAMES[contrastive_type]

        for col, text in pair.items():
            if col not in valid_cols:
                continue
            if isinstance(text, list):
                tokenized[col] = [self.preprocess_text(t, col, ds_name) for t in text]
            else:
                tokenized[col] = self.preprocess_text(text, col, ds_name)

        return tokenized


    def load_hf_dataset(self, ds_spec, data_path=None, num_workers=0):
        dataset_name = ds_spec["dataset_name"]
        all_splits = [split["name"] for split in ds_spec["splits"]]
        ...
        split2prefix = {
            'default': {
                "query": ds_spec.get("default_query_prefix", "query"),
                "document": ds_spec.get("default_document_prefix", "passage"),
            }
        }

        for split in ds_spec["splits"]:
            if split.get("query_prefix", None):
                name = split["name"]
                split2prefix[name] = {
                    "query": split["query_prefix"],
                    "document": split.get("document_prefix", split["query_prefix"]),
                }

        contrastive_type = ds_spec["objective"]["type"]
        columns = ds_spec["objective"]["columns"]

        dataset = dataset.rename_columns(dict(zip(columns, MAPPED_NAMES[contrastive_type])))

        return dataset, split2prefix


def collate_hf_ds(batch, tokenizer, max_lengths=None):

    cols = list(batch[0].keys())
    batch_input = {}
    for col in cols:
        # if batch[0][col] is a list of lists, flatten it
        if isinstance(batch[0][col], list):
            collected = [item for sublist in [sample[col] for sample in batch] for item in sublist]
        else:
        ...
        tokenized = tokenizer(
            ...
            truncation=True,
            return_tensors="pt",
            max_length=max_lengths[col] if max_lengths else None,
        )

        batch_input.update({f"{col}_{k}": v for k, v in tokenized.items()})
...
class StreamingHFDataset(IterableDataset):
    def __init__(self, config_file, tokenizer, num_negatives=0, add_prefix=True, add_eos=False, data_path=None, query_max_length=None, document_max_length=None, num_workers=0, shuffle_buffer_size=10000, rank=0, world_size=1):
        self.config_file = config_file
        self.tokenizer = tokenizer
        self.num_negatives = num_negatives
        self.add_prefix = add_prefix
        ...
        self.dataset, self.split2prefix = self.load_hf_dataset(self.ds_spec, data_path)
    ...
    def __iter__(self):
        for sample in self.dataset:
            # Map the column names to the expected format
            sample = {self.column_map.get(k, k): v for k, v in sample.items()}

            # Build paired data from the sample, if it's negative, it will be added to the document
            pair = self.build_paired_data(sample)
    ...
    def load_hf_dataset(self, ds_spec, data_path=None):
        dataset_name = ds_spec["dataset_name"]
        splits = [split["name"] for split in ds_spec["splits"]]


        contrastive_type = ds_spec["objective"]["type"]
        columns = ds_spec["objective"]["columns"]
        self.column_map = dict(zip(columns, MAPPED_NAMES[contrastive_type]))
        ...
        num_examples_per_split = [builder.info.splits[split].num_examples for split in splits]
        self.num_examples = sum(num_examples_per_split)
        ...
        for split, ne in zip(splits, num_examples_per_split):
            print(f"Split: {split}, Number of examples: {ne} ({ne / self.num_examples:.2%})")

        if len(data_splits) == 1:
            combined_dataset = data_splits[0]
        else:
            combined_dataset = interleave_datasets(
                data_splits,
                stopping_strategy='all_exhausted',
                probabilities=[ne / self.num_examples for ne in num_examples_per_split],
            )

        shuffled_dataset = combined_dataset.shuffle(seed=42, buffer_size=self.shuffle_buffer_size)

        if self.world_size > 1:
            final_dataset = split_dataset_by_node(shuffled_dataset, rank=self.rank, world_size=self.world_size)
        else:
            final_dataset = shuffled_dataset

        return final_dataset, split2prefix


class StreamingMosaicDataset(IterableDataset):
    def __init__(self, config_file, tokenizer, batch_size, num_negatives=0, add_prefix=True, add_eos=False, data_path=None, query_max_length=None, document_max_length=None, num_workers=0, shuffle_buffer_size=10000, rank=0, world_size=1, **kwargs):
        self.config_file = config_file
        self.tokenizer = tokenizer
        ...
        self.dataset, self.split2prefix = self.load_dataset(self.ds_spec, data_path)
    ...
    def load_dataset(self, ds_spec, data_path=None):
        dataset_name = ds_spec["dataset_name"]
        data_path = ds_spec.get("data_path", data_path)
        splits = [split["name"] for split in ds_spec["splits"]]
        ...
        num_examples_per_split = [metadata["split_infos"][split]['num_examples'] for split in splits]
        self.num_examples = sum(num_examples_per_split)
        proportions = [ne / self.num_examples for ne in num_examples_per_split]
        for split, ne in zip(splits, num_examples_per_split):
            print(f"Split: {split}, Number of examples: {ne} ({ne / self.num_examples:.2%})")


        data_splits = [
            Stream(local=os.path.join(data_path, f"{split}"), proportion=prop)
            for split, prop in zip(splits, proportions)
        ]
        final_dataset = StreamingDataset(
            streams=data_splits,
...
def get_local_dataloader(config_file, batch_size, tokenizer, num_negatives, seed, add_prefix=True, add_eos=False, streaming=None, num_workers=0, epoch=0, data_path=None, query_max_length=None, document_max_length=None, shuffle_buffer_size=1000000):
    ...
    if streaming == 'hf':
        ...
        sampler = None  # Streaming datasets do not use samplers
    elif streaming == 'mosaicml':
        dataset = StreamingMosaicDataset(
            config_file=config_file,
            tokenizer=tokenizer,
        ...
        if dist.is_initialized():
            sampler = DistributedSampler(dataset, shuffle=True, num_replicas=dist.get_world_size(), rank=dist.get_rank(), seed=seed)
            sampler.set_epoch(epoch)
        else:
            sampler = None

    collate_fn = partial(collate_hf_ds, tokenizer=tokenizer, max_lengths=dataset.max_lengths)
